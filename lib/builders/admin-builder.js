...
class AdminBuilder extends BaseBuilder {
  async createProject(dir, { name, prefix, domain, port }) {
    ...
    console.log("create project", projectName.lower);

    this.mkdirs(dir, ["bin", "data", "data/config", "data/config/conf.d", "data/constants", "env", "public", "utils"]);
  ...
  async createModel(dir, { model, connection }) {
    ...
    this.createTemplateFile(dir, {
      source: "api/model/mapper.xml.ejs",
  ...
  async generateProject() {
    ...
    return this.createProject(path.resolve("."), { name, prefix, domain, port });
  }
